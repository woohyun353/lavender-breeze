...
async function getRoomPageData(roomId: string): Promise<{
  room: Pick<Room, "id" | "slug" | "title" | "subtitle" | "description" | "cover_image_url" | "exhibition_id" | "type" | "order">;
  siblingRooms: { id: string; slug?: string | null; title: string; order: number | null }[];
  items: GalleryItem[];
  posts: Post[];
} | null> {
  ...
  const [siblingResult, contentResult] = await Promise.all([
    supabase
      .from("rooms")
  ...
  const siblingRooms = (siblingResult.data as { id: string; slug?: string | null; title: string; order: number | null }[]) ?? [];
  let items: GalleryItem[] = [];
  let posts: Post[] = [];
  if (room.type === "image") {
    const rows = (contentResult.data as { id: string; image_url: string | null; caption: string | null; description?: string | null }[]) ?? [];
    items = rows.map((r) => ({
      id: r.id,
    ...
    posts = (contentResult.data as Post[]) ?? [];
  }
  return { room, siblingRooms, items, posts };
}

const getCachedRoomPage = (roomId: string) =>
  unstable_cache(() => getRoomPageData(roomId), ["room-page", roomId], { revalidate: 60 })();

export default async function RoomPage({ params }: Props) {
  const { roomId } = await params;
  let data = await getCachedRoomPage(roomId);
  ...
  if (data === null) {
          ...
          <main className="min-h-screen bg-[var(--color-bg)]">
            <header className="border-b border-black/5 bg-[var(--color-panel)]">
              <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4 sm:px-10 sm:py-5">
                <Link href="/exhibitions" className="text-sm text-body/70 hover:text-body">← 전시 목록</Link>
    ...
    data = await getRoomPageData(roomId);
    if (data === null) notFound();
  }

  const { room, siblingRooms, items, posts } = data;
  if (isUuid(roomId) && room.slug && !isUuid(room.slug)) {
    redirect(`/rooms/${room.slug}`);
  }
  const finalItems =
    room.type === "image" && items.length === 0 && DUMMY_FALLBACK[roomId]
      ? DUMMY_FALLBACK[roomId].items
      : items;
    ...
    <main className="min-h-screen bg-[var(--color-bg)]">
      ...
      <div className="mx-auto max-w-6xl px-6 sm:px-10">
        {isTextRoom ? (
          ...
          <RoomGallery roomTitle={room.title} items={finalItems} />
        )}
      </div>
    </main>
  );
